...
@pytest.mark.parametrize("original_number, masked_number", [("7000792289606361", "7000 79** **** 6361"),
                                                            ("1234567890123456", "1234 56** **** 3456")])
def test_mask_card_or_account(original_number, masked_number):
    assert mask_credit_card_number(original_number) == masked_number


@pytest.mark.parametrize("original_number, masked_number", [("73654108430135874305", "**4305"), ("1234567890123456", "**3456")])
def test_form_mask_credit_card_number(original_number, masked_number):
    assert mask_account_number(original_number) == masked_number


if __name__ == "__main__":
    pytest.main()
    test_mask_card_or_account()
    test_form_mask_credit_card_number()
